...
/*   Updated: 2025/05/13 19:15:56 by jisokim2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	open_redir_file(char *filename, int flags, mode_t mode)
{
	...
	if (fd == -1)
	{
		printf("minishell: %s: %s\n", filename, strerror(errno)); // do we need? correct message?
		return (fd);
	}
	return (fd);
}
...
static int	dup2_redir(int oldfd, int newfd)
{
	...
	{
		printf("minishell: dup2(%d: %d), %s\n", oldfd, newfd, strerror(errno)); // do we need? correct message?
		return (-1);
	}
	return (0);
}
...
static int	handle_redir(t_io_list	*io_streams)
{
	...
	if (io_streams->infile_name)
	{
		if (hoo (io_streams->infile_name, STDIN_FILENO, O_RDONLY, 0) < 0)
			return -1;
		// if (open_redir_file(&fd, io_streams->infile_name, O_RDONLY, 0) < 0)
		// 	return -1;
		// if (dup2_redir(fd, STDIN_FILENO) < 0)
		// 	return -1;
		// close(fd);
	}
	if (io_streams->outfile_name)
	{
		if (hoo (io_streams->outfile_name, STDOUT_FILENO, O_WRONLY | O_CREAT | O_TRUNC, 0644) < 0)
			return -1;
		// if (open_redir_file(&fd, io_streams->outfile_name, O_WRONLY \
		// 	| O_CREAT | O_TRUNC, 0644) < 0)
		// 	return -1;
		// if (dup2_redir(fd, STDOUT_FILENO) < 0)
		// 	return -1;
		// close(fd);
	}
	if (io_streams->append_file_name)
	{
		if (hoo (io_streams->append_file_name, STDOUT_FILENO, O_WRONLY | O_CREAT | O_APPEND, 0644) < 0)
			return -1;
		// if (open_redir_file(&fd, io_streams->append_file_name, O_WRONLY \
